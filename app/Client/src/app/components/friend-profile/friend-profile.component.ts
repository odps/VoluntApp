...
export class FriendProfileComponent implements OnInit {
  ...
  activeSection: string = 'default'; // Definir la propiedad activeSection

  isDesktop: boolean = true; // Definir la propiedad isDesktop
  ...
  responderPeticionAmistad(friendId: number, status: 'accepted' | 'rejected') {
    this.friendService.respondFriendshipRequest(friendId, status).subscribe({
      next: (response) => {
        console.log(
          'Ha respondido a la peticion de amistad con: ' +
            status +
            '  ' +
            response,
        );
  ...
  // Definir el método changeSection
  changeSection(section: string): void {
    this.activeSection = section;
  }

  // Definir el método goBack
  goBack(): void {
    this.router.navigate(['/previous-page']); // Ajusta la ruta según sea necesario
  }
}
